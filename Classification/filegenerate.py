...
data_mean_combined = np.zeros(9)
data_var_combined = np.zeros(9)
...
mean = np.zeros((len(range(0, row, STEP)) , col))
var = np.zeros((len(range(0, row, STEP)) , col))

for i_col in range(col):
	idx = 0
	for i_row in range(0, row, STEP):
	# compute mean of window elements
	mean[idx,i_col] = np.mean(data_all[i][i_row:i_row+WINDOW_SIZE-1,i_col])
        var[idx,i_col] = np.var(data_all[i][i_row:i_row+WINDOW_SIZE-1,i_col])
        idx += 1
