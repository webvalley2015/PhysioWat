...
from .views import show_chart, select_experiment, select_record, test
...
urlpatterns = patterns('preproc.views',
    ...
    url( regex='^linegraph_getdata/(?P<id_num>\d+)/(?P<elab>\w+)/$', view=linegraph.as_view(), name="chart_getdata" ),
    url( regex='^chart/(?P<id_num>\d+)/(?P<alg_type>\d*)/{0,1}$', view=show_chart, name="chart_show" ),
    url( regex='^test$', view=test, name="Test"),
    url( regex='^linegraph_getdata1/(?P<id_num>\d+)/(?P<elab>\w+)/$', view=linegraph2.as_view(), name="chart_getdata1" ),
