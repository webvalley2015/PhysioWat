...
def scatter_error_bar_creator_2(accuracy, error):
    ...
    mat_modified = np.empty([0,3])
    for index, item in np.ndenumerate(accuracy):
        mat_modified = np.vstack((mat_modified, [index[0], index[1], item]))
    mat_modified = np.hstack((mat_modified, (accuracy - error).flatten().reshape((12,1))))
    mat_modified = np.hstack((mat_modified, (accuracy + error).flatten().reshape((12,1))))
    return mat_modified
